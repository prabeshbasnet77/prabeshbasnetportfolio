...
export default function App() {
  ...
  // Initialize and load persistent memory states
  useEffect(() => {
  ...
  // Projects mutations
  const handleUpdateProjectPublic = (id: string, isPublic: boolean) => {
    const list = projects.map(p => p.id === id ? { ...p, isPublic } : p);
    syncProjects(list);
  };

  const handleAddProject = (p: Omit<Project, 'id' | 'dateAdded'>) => {
  ...
  const handleUpdateProject = (p: Project) => {
    const list = projects.map(item => item.id === p.id ? p : item);
    syncProjects(list);
  };
  ...
  // Dispatch public contact form values directly into administrator messages list
  ...
  return (
    <>
      {/* GL Fluid Shader Atmosphere in background */}
      <ShaderBackground />
      ...
      {isAdmin ? (
        !isLoggedIn ? (
          ...
          /* ADMN CONSOLE CONTROL CENTER INTERFACE */
          <div className="min-h-screen flex flex-col md:flex-row font-sans text-on-background select-none overflow-x-hidden">
          {/* Main left railway pane */}
          <aside className="w-full md:w-64 bg-[#15121bd9] backdrop-blur-3xl border-r md:border-b-0 border-white/[0.06] flex flex-col justify-between p-4 gap-6 shrink-0 shrink-none">
            ...
            <div className="pt-4 border-t border-white/[0.04] space-y-2">
              <button
                id="sidebar-view-site-cta"
                onClick={() => setIsAdmin(false)}
                className="w-full py-2.5 rounded-xl bg-white/[0.05] hover:bg-white/[0.08] text-white border border-white/[0.08] text-xs font-semibold select-none transition-all flex items-center justify-center gap-1.5 cursor-pointer"
              >
              ...
              <button
                id="sidebar-logout-cta"
                onClick={() => {
                  setIsLoggedIn(false);
                  localStorage.removeItem('pb_admin_logged_in');
                }}
                className="w-full py-2.5 rounded-xl bg-red-400/5 hover:bg-red-500/15 text-red-400 border border-red-500/15 text-xs font-semibold select-none transition-all flex items-center justify-center gap-1.5 cursor-pointer active:scale-95"
              >
          ...
          <main className="flex-1 bg-black/40 backdrop-blur-md p-6 md:p-10 flex flex-col justify-between gap-12 overflow-y-auto">
            ...
            <header className="flex items-center justify-between border-b border-white/[0.04] pb-4 shrink-0">
              ...
              <button
                id="top-back-site-cta"
                onClick={() => setIsAdmin(false)}
                className="px-4 py-2 rounded-xl bg-primary text-black text-xs font-bold hover:opacity-90 active:scale-95 transition-all cursor-pointer flex items-center gap-1 shadow-[0_4px_12px_rgba(208,188,255,0.2)]"
              >
            ...
            {/* Dynamic workspace components selector */}
            ...
            <footer className="border-t border-white/[0.04] pt-4 font-mono text-[9px] text-[#cbc3d7]/40 text-left shrink-0">
              Workspace terminal instance synced in localStorage variables. Node 18.2 Web Engine.
            </footer>
          </main>
        </div>
        )
      ) : (
        /* PUBLIC-FACING LIVE PORTFOLIO WEBSITE */
        <LivePortfolio
          profile={profile}
          socials={socials}
          projects={projects}
      ...
      {/* Persistent global Floating Navigation switch indicator */}
      <div className="fixed bottom-6 right-6 z-50">
        <button
          id="global-workspace-switch"
          onClick={() => {
            if (isAdmin) {
              setIsAdmin(false);
            } else {
